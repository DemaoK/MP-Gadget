...
set_sidm_bhseed_params(ParameterSet * ps)
{
    ...
    if(ThisTask == 0) {
        ...
        if(sidm_bhseed_params.SeedOn && !param_get_int(ps, "BlackHoleOn"))
            endrun(1, "SIDMBHSeedOn requires BlackHoleOn.\n");
...
sidm_bhseed_dm_swallow_haswork(int n, TreeWalk * tw)
{
    ...
    return P[n].Type == 5 && !P[n].Swallowed &&
        ...
        FORCE_SOFTENING_PAIR(5, 1) > 0;
}

static double
sidm_bhseed_dm_swallow_kernel_length(void)
{
    return FORCE_SOFTENING_PAIR(5, 1);
}
