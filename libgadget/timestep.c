...
void
grav_short_tree_build_tree(const ActiveParticles * subact, PetaPM * pm, const PMZoomRegion * pmzoom, DomainDecomp * ddecomp, MyFloat (* AccelStore)[3], inttime_t Ti_Current, const double rho0, int HybridNuGrav, const char * EmergencyOutputDir)
{
    ...
    grav_short_tree(subact, pm, pmzoom, &Tree, AccelStore, rho0, Ti_Current);
...
int
hierarchical_gravity_and_timesteps(const ActiveParticles * act, PetaPM * pm, const PMZoomRegion * pmzoom, DomainDecomp * ddecomp, struct grav_accel_store StoredGravAccel, DriftKickTimes * times, const double atime, int HybridNuGrav, int FastParticleType, Cosmology * CP, const char * EmergencyOutputDir)
{
    ...
    if(isPM) {
        ...
        if(pmzoom && pmzoom->Enabled && pmzoom->Asmth > 0 && pmzoom->Asmth < asmth)
            asmth = pmzoom->Asmth;
     ...
     */
    //grav_short_tree_build_tree(subact, pm, pmzoom, ddecomp, StoredGravAccel, times->Ti_Current, rho0, HybridNuGrav, EmergencyOutputDir);
    ...
    for(ti = largest_active-1; ti > 0; ti--) {
        ...
        grav_short_tree_build_tree(subact, pm, pmzoom, ddecomp, GravAccel, times->Ti_Current, rho0, HybridNuGrav, EmergencyOutputDir);
...
int hierarchical_gravity_accelerations(const ActiveParticles * act, PetaPM * pm, const PMZoomRegion * pmzoom, DomainDecomp * ddecomp, struct grav_accel_store StoredGravAccel, DriftKickTimes * times, int HybridNuGrav, Cosmology * CP, const char * EmergencyOutputDir)
{
    const double rho0 = CP->Omega0 * 3 * CP->Hubble * CP->Hubble / (8 * M_PI * CP->GravInternal);
...
//     message(0, "Starting gravity accelerations: largest %d\n", largest_active);
    ...
    grav_short_tree_build_tree(lastact, pm, pmzoom, ddecomp, StoredGravAccel.GravAccel, times->Ti_Current, rho0, HybridNuGrav, EmergencyOutputDir);
...
get_timestep_gravity_dloga(const int p, const MyFloat * const GravAccel, const double atime, const double hubble)
{
    ...
    double dt = sqrt(2 * TimestepParams.ErrTolIntAccuracy * atime * (FORCE_SOFTENING_TYPE(P[p].Type) / 2.8) / ac);
